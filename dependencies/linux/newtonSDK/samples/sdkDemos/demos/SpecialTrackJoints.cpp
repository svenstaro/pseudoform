...
//int xxx;
//xxx = NewtonWorldGetBodyCount(world);
//xxx = NewtonWorldGetConstraintCount(world);
//xxx = NewtonWorldGetConstraintCount(world);
	...
	public:
	FrictionTrank(const char* name, SceneManager* system, NewtonWorld* nWorld, const dMatrix& matrix, int bodyID, int tracksID)
		:RenderPrimitive(matrix, NULL)
	{
		...
		LoadCollada(fullPathName, context);
		system->Append (this);	
...
*/
	...
	static void SetTransform (const NewtonBody* body, const dFloat* matrix, int threadIndex)
	{
		...
		dMatrix threadMatrix;
		dMatrix invMatrix (me->m_matrix.Inverse());

		NewtonBodyGetMatrix (me->m_leftThreadBody, &threadMatrix[0][0]);
		me->m_leftThread->m_localMatrix = threadMatrix * invMatrix;

		NewtonBodyGetMatrix (me->m_rightThreadBody, &threadMatrix[0][0]);
		me->m_rightThread->m_localMatrix = threadMatrix * invMatrix;
	}
	...
	NewtonBody* CreateBodyPart (dBone* bone, NewtonWorld* nWorld, dFloat mass) const
	{
		...
		dMatrix matrix (bone->CalcGlobalMatrix() * m_matrix);
...
//	NewtonBody* floor; 
	...
	for (int x = 0; x < 3; x ++) {
		for (int z = 0; z < 3; z ++) {
			...
			new FrictionTrank ("m1a1.dae", &system, system.m_world, matrix, defaultID, tracksID);
		}
	}
}
