...
static NewtonCollision* CreateBarrelHull (SceneManager& system)
{
	dInt32 i;
	dInt32 j;
	dInt32 size;
...
static void CreateWall (SceneManager& system, NewtonCollision* collision, dVector origin, int high)
{
	dFloat mass;
	dFloat Ixx;
	dFloat Iyy;
...
void CreateWalls (SceneManager& system)
{
	...
	CreateWall (system, collision, dVector(-20.0f, 0.0f, -10.0f, 0.0f), 10);
	NewtonReleaseCollision (system.m_world, collision);

	collision = NewtonCreateBox (system.m_world, 1.0f, 1.0f, 2.0f, 0, NULL); 
	CreateWall (system, collision, dVector(-10.0f, 0.0f, 0.0f, -10.0f), 10);
	NewtonReleaseCollision (system.m_world, collision);	

	collision = CreateBarrelHull (system); 
	CreateWall (system, collision, dVector( 0.0f, 0.0f, -10.0f, 0.0f), 10);
	NewtonReleaseCollision (system.m_world, collision);	
