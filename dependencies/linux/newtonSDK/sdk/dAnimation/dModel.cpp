...
dMesh* dLoaderContext::CreateMesh ()
{
	return new dMesh ();
}
...
//		m_meshList.Append();
//	}
//	ModelComponentList<dList<dMeshInstance> >::dListNode* list = m_meshList.GetFirst();
//	list->GetInfo().m_data.Append (dMeshInstance (mesh));
//	mesh->AddRef();
...
void dModel::AddSkeleton (dBone* rooBone)
{
	ModelComponent<dList<dBone*> >& boneList = m_skeleton.Append()->GetInfo();
	boneList.m_data.Append (rooBone);
}
...
/*
	for (ModelComponentList<dList<dMeshInstance> >::dListNode* list = m_meshList.GetFirst(); list; list = list->GetNext()) {
		for (dList<dMeshInstance>::dListNode* node = list->GetInfo().m_data.GetFirst(); node; node = node->GetNext()) { 
			...
			mesh = node->GetInfo();
			if (mesh->GetModifier()) {
				mesh->GetModifier()->ApplyGlobalScale(scale);
			}
		}
	}
*/
...
//			mesh = node->GetInfo();
//			if (mesh->GetModifier()) {
//				mesh->GetModifier()->ApplyGlobalTransform(transform);
//			}
//		}
//	}
}
